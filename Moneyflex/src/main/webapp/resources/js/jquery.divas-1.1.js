				...
				this.each(function() {
					...
					$right_wing.css({
						...
						"left": "auto"
												...
												// prev, next Button settings
					$prev.css({
							"left": Math.round((slider_width - slide_width)/2),
	
							"background" : "url("../images/prev.svg") no-repeat top center rgba(0, 0, 0, 0)"
					});

					$next.css({
							"right": Math.round((slider_width - slide_width)/2),
			...
			resize: function(id) {
				...
				if(data !== undefined)
				{
					var $slider = data.slider,
						...
						slide_width = inpx(settings.mainImageWidth,slider_width),
						container_width = tot_slides * slide_width,
						wing_width = Math.round((slider_width - slide_width)/2) * 1.2,
						slide_height = Math.round(slide_width * img_ratio),
						slide_left = Math.round((slider_width - slide_width)/2) - (slide_width * current),
						slideTransitionParameters = data.slideTransitionParameters;
				
					$slider.css({"width": slider_width});
					$container.css({"width": container_width});
					$left_wing.css({"width": wing_width,"height": "640px"});
					$right_wing.css({"width": wing_width,"height": "640px"});
